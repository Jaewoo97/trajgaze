"""
PLLaVA-7B + TrajGaze encoder (10% tokens) + LoRA r=64, logit-based MCQ + KD.

Teacher : finetuned PLLaVA (64.8%, pllava_baseline_lora/best_delta.pth), full 2304 tokens, frozen.
Student : PLLaVA + LoRA r=64 (fresh) + TrajGaze-merged 10% tokens, trainable.

Training protocol matches the 64.83% baseline:
  - Logit-based MCQ loss: CE at ASSISTANT: position over full LLaMA vocabulary,
    ground-truth = correct option letter token ID (A=319, B=350, C=315, D=360).
  - KD: KL(student_opts / T || teacher_opts / T) over the 4 option logits.
  - Combined: loss = (1-alpha)*CE + alpha*KL*T^2

Trainable: LoRA r=64 adapters + TrajGaze encoder.
Frozen   : vision tower, base LLM weights.

Train : egoexolearn + holoassist (5799 items)
Eval  : egtea (526 items, logit-based with merged tokens)
GPUs  : 0,1,2,3 via torchrun

Usage:
    CUDA_VISIBLE_DEVICES=0,1,2,3 /opt/conda/envs/gaze/bin/torchrun \
        --nproc_per_node=4 --master_port=29800 \
        -m TrajGazeMerge.training.train_pllava_trajgaze_kd_r64 \
        --output-dir /workspace/EgoGazeVQA/TrajGazeMerge/checkpoints/pllava_trajgaze_kd_r64 \
...
LORA_R     = 64
LORA_ALPHA = 128   # scale = 2.0, matches the 64.83% teacher training
...
def _build_pllava_base(device):
    """Load PLLaVA-7B with merged HF PEFT weights. Returns (model, processor)."""
...
def load_teacher(teacher_ckpt_path: str, device: torch.device):
    ...
    # Add LoRA r=64 adapters to accept the saved delta keys
    model.language_model = get_peft_model(model.language_model, _lora_config(inference_mode=True))
    # Load finetuned delta (lora_A/B + projector weights)
...
def load_student(device: torch.device):
    """Load fresh PLLaVA + LoRA r=64 student (trainable LoRA only)."""
...
def merge_visual_features(full_features, scores, device):
    """Merge 90% of visual tokens. Returns (1, ~230, d)."""
    N_vis = full_features.shape[1]
    n_spatial  = POOLING_SHAPE[1] * POOLING_SHAPE[2]  # 144
    n_temporal = POOLING_SHAPE[0]                      # 16
    scores_sp  = score_to_pllava_spatial(scores, n_spatial)          # (144,)
    scores_all = scores_sp.unsqueeze(0).expand(n_temporal, -1).reshape(-1)  # (2304,)
...
def forward_logit(model, processor, image_features, question, options, device):
    """Inject image_features, forward, return logit at ASSISTANT: position."""
    model_dtype = next(model.language_model.parameters()).dtype
...
def parse_args():
    ...
    p.add_argument("--output-dir",    default="/workspace/EgoGazeVQA/TrajGazeMerge/checkpoints/pllava_trajgaze_kd_r64")
...
def main():
    ...
    if is_main:
        ...
        print(f"[PLLaVA TrajGaze KD r64] output: {args.output_dir}")
    ...
    # ── Load student (fresh PLLaVA + LoRA r=64) ──
    if is_main:
        print("Loading student (PLLaVA + LoRA r=64, fresh) ...")
    student, processor = load_student(device)
    ...
    # ── Option token IDs (A=319, B=350, C=315, D=360) ──
    tok = processor.tokenizer
    ...
    for epoch in range(args.epochs):
        ...
        for step, item in enumerate(loader):
            ...
            try:
                ...
                # ── Full visual features (no grad; shared by teacher + student merge) ──
                with torch.no_grad():
                ...
                # ── GT index among 4 options ──
                gt_letter = item["answer"].upper()
                if gt_letter not in OPTION_LETTERS:
                    continue
                gt_idx = OPTION_LETTERS.index(gt_letter)
                gt_tensor = torch.tensor([gt_idx], device=device, dtype=torch.long)

                # ── Option-only 4-class CE loss (directly aligns with eval) ──
                student_opts_ce = logit_student[option_ids_t]  # (4,)
                loss_ce = F.cross_entropy(student_opts_ce.unsqueeze(0), gt_tensor)

                # ── KD loss: KL over 4 option logits ──
                T = args.kd_temp
                s_opts = logit_student[option_ids_t] / T  # (4,)
                t_opts = teacher_opts / T                  # (4,)
                loss_kd = F.kl_div(
                ...
                if is_main and n_steps % args.log_every == 0:
                    ...
                    print(f"Epoch {epoch+1} | step {n_steps}/{len(loader)} | "
                          f"loss={avg_l:.4f} ce={avg_ce:.4f} kd={avg_kd:.4f} | t={elapsed:.0f}s")
                    with open(log_path, "a") as f:
                        f.write(json.dumps({
                            "epoch": epoch + 1, "step": n_steps,
                            "loss": avg_l, "ce": avg_ce, "kd": avg_kd, "elapsed": elapsed,
                        }) + "\n")
                ...
                if is_main and n_steps % args.eval_every == 0:
                    ...
                    print(f"  → eval_acc_logit (merged, partial): {acc:.2f}% (n={n_eval})")
                    if acc > best_acc:
                        best_acc = acc
                        torch.save({
                            "epoch": epoch, "step": n_steps,
                            "lora_state": {k: v.cpu() for k, v in student.module.state_dict().items()
        ...
        if is_main:
            ...
            print(f"\n=== Epoch {epoch+1}/{args.epochs} | "
                  f"avg_loss={avg_l:.4f} ce={avg_ce:.4f} kd={avg_kd:.4f} | time={elapsed:.0f}s ===")

            acc, n_eval = evaluate_logit(
                student.module, processor, traj_encoder.module, device,
                option_ids_t, n_frames=args.n_frames,
            )
            print(f"  → eval_acc_logit (merged, full): {acc:.2f}% (n={n_eval})")
