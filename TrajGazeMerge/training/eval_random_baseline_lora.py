"""
Random 10% token selection (no merging) applied at inference to baseline_lora.

Randomly selects 10% of visual tokens; bottom 90% are discarded.
No retraining — baseline_lora checkpoint used as-is.

Usage:
    CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 --master_port=29608 \\
        -m TrajGazeMerge.training.eval_random_baseline_lora \\
        --out /workspace/EgoGazeVQA/TrajGazeMerge/eval_results/random_baseline_lora_metrics.json
"""
...
def main():
    ...
    if is_main:
        print(f"[RandBaseline] keep={args.keep_ratio*100:.0f}% random, ckpt={args.ckpt}", flush=True)
    ...
    with torch.no_grad():
        for item in loader:
            ...
            try:
                ...
                receiver_idx, _ = torch.randperm(N, device=device)[:k].sort()
                selected = cached["video_embeds"][receiver_idx]

                inputs_dict   = build_merged_inputs(base_qwen, cached, selected, receiver_idx)
    ...
    if is_main:
        ...
        print(f"\n=== Random 10% on baseline_lora ===", flush=True)
        ...
        with open(args.out, "w") as f:
            json.dump({"acc": acc, "n": stats[1].item(), "per_task": per_task,
                       "ckpt": args.ckpt, "keep_ratio": args.keep_ratio,
                       "method": "random_drop_baseline_lora"}, f, indent=2)
