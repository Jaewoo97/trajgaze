#!/usr/bin/env bash
# E1 patch-temporal stage-2 with keep=3% (merge_ratio=0.97).
# Reuses the existing E1_patch_temporal stage-1 encoder ckpt — no Stage 1 retrain.
# Stage-2 only: token-merge + Qwen LoRA at 3% retention.
#
# Paper main table: token retention {10%, 5%, 3%}.
#   10% (merge_ratio=0.9):   E1_patch_temporal_keep10_bs4 → best 68.44% (Ep2 step 4400)
#   5%  (merge_ratio=0.95):  E1_patch_temporal_keep05_bs4 → best 66.54% (Ep2 step 4000)
#   3%  (merge_ratio=0.97):  THIS RUN → E1_patch_temporal_keep03_bs4
#
...
S2_OUT=$REPO/TrajGazeMerge/checkpoints/E1_patch_temporal_keep03_bs4
mkdir -p "$S2_OUT"

LAUNCHER=$S2_OUT/launcher.log
echo "[$(date)] E1 patch-temporal keep=3% pipeline launched (GPU 0, detached)" > "$LAUNCHER"
...
# ── Stage 2: token-merge + Qwen LoRA (merge_ratio=0.97 → keep 3%) ─────────────
echo "[$(date)] Stage-2: training token-merge + Qwen LoRA (keep=3%) ..." >> "$LAUNCHER"
