"""
PLLaVA-7B + TrajGazeV2Temporal encoder (10% tokens) + LoRA r=16, logit-based MCQ.

Temporal variant of train_pllava_trajgaze_lora.py:
- TrajGazeV2Temporal returns (T_traj, 196) per-frame scores (vs flat 196 spatial).
- score_to_pllava_spatiotemporal maps (T_traj,196) → (2304,) matching PLLaVA's
  POOLING_SHAPE = (16, 12, 12): T_traj→16 linear, 14×14→12×12 bilinear.
- Same logit-based MCQ loss (CE at ASSISTANT: token over full vocab).
- Single GPU (GPU 3).

Train : egoexolearn + holoassist (5799 items)
Eval  : egtea (526 items, logit-based with 10%-merged tokens)
GPU   : single GPU

Usage:
    CUDA_VISIBLE_DEVICES=3 /opt/conda/envs/gaze/bin/python \
        -m TrajGazeMerge.training.train_pllava_trajgaze_temporal_lora \
        --stage1-ckpt /workspace/EgoGazeVQA/TrajGaze_v2/checkpoints/stage1_temporal/best.pth \
        --output-dir /workspace/EgoGazeVQA/TrajGazeMerge/checkpoints/pllava_trajgaze_temporal_lora \
...
def load_traj_encoder(ckpt_path: str, device: torch.device, n_vis_keyframes: int = 16):
    ...
    if os.path.exists(ckpt_path):
        ...
        print(f"[TrajEncTemporal] Loaded | missing={len(missing)} unexpected={len(unexpected)}")
    else:
        print(f"[TrajEncTemporal] WARNING: ckpt not found: {ckpt_path}, using random init")
    return enc


def score_to_pllava_spatiotemporal(scores: torch.Tensor) -> torch.Tensor:
    """
    scores: (T_traj, 196) → (TOTAL_VIS_TOKENS,) = (2304,)
    Maps T_traj → T_vis=16 temporal, 14×14 → 12×12 spatial.
    """
    T_traj = scores.shape[0]
    T_vis   = POOLING_SHAPE[0]          # 16
    n_side  = POOLING_SHAPE[1]          # 12
    n_spatial = n_side * n_side         # 144

    # Spatial: 14×14 → 12×12 bilinear
    ...
    # Temporal: T_traj → T_vis linear interp
    s = s.reshape(T_traj, n_spatial).t().unsqueeze(0)      # (1, 144, T_traj)
    s = F.interpolate(s, size=T_vis, mode="linear",
                      align_corners=False).squeeze(0)      # (144, T_vis)
    return s.t().contiguous().view(-1)                     # (T_vis*144,) = (2304,)
...
def get_patch_scores_temporal(traj_encoder, item, device):
    """Returns (T_traj, 196) per-frame patch scores from TrajGazeV2Temporal."""
...
def merge_features_temporal(full_features, scores_temporal, device):
    """Merge 90% of PLLaVA visual tokens guided by temporal TrajGaze scores."""
    N_vis = full_features.shape[1]
    scores_all = score_to_pllava_spatiotemporal(scores_temporal)  # (2304,)
...
def forward_logit(model, processor, image_features, question, options, device):
    """Forward with merged image features; returns logit at last (ASSISTANT:) position."""
    model_dtype = next(model.language_model.parameters()).dtype
...
def evaluate_logit(model, processor, traj_encoder, device, option_ids_t,
                   n_frames=16, n_traj_frames=128, max_items=None):
    """Logit-based eval on egtea test split with 10%-merged temporal tokens."""
    ...
    with torch.no_grad():
        for item in test_ds:
            ...
            try:
                ...
                merged = merge_features_temporal(full_features, scores, device)
...
def parse_args():
    ...
    p.add_argument("--output-dir",      default="/workspace/EgoGazeVQA/TrajGazeMerge/checkpoints/pllava_trajgaze_temporal_lora")
...
def main():
    ...
    device = torch.device("cuda:0")  # mapped via CUDA_VISIBLE_DEVICES=3
    os.makedirs(args.output_dir, exist_ok=True)
    print(f"[PLLaVA TrajGaze Temporal LoRA] output: {args.output_dir}")
    print(f"epochs={args.epochs}, lr_lora={args.lr_lora}, lr_enc={args.lr_enc}, "
          f"grad_accum={args.grad_accum}, seed={args.seed}")

    print("Loading PLLaVA-7B + LoRA r=16 ...")
    model, processor = load_pllava(device)

    print("Loading TrajGazeV2Temporal encoder ...")
    ...
    tok = processor.tokenizer
    # Use standalone letter token IDs (A=319, B=350, C=315, D=360 in LLaMA tokenizer)
    # matching the successful 60.46% baseline — NOT context-encoded which gives wrong merged tokens
    option_ids = [tok.encode(l, add_special_tokens=False)[-1] for l in OPTION_LETTERS]
    ...
    for epoch in range(args.epochs):
        ...
        for step, item in enumerate(loader):
            ...
            try:
                ...
                # Full visual features (vision tower frozen)
                with torch.no_grad():
                    full_features = get_pllava_image_features(model, pixel_values)

                # Temporal TrajGaze scores (grad flows through encoder)
                scores = get_patch_scores_temporal(traj_encoder, item, device)

                # Merge to 10%
                merged = merge_features_temporal(full_features, scores, device)

                # Forward → logit at ASSISTANT: position
                logit_last = forward_logit(model, processor, merged,
                                           item["question"], item["options"], device)

                # Logit-based MCQ CE loss
                ...
                if n_steps % args.eval_every == 0:
                    acc, n_eval, per_task = evaluate_logit(
                        model, processor, traj_encoder, device,
                        option_ids_t, n_frames=args.n_frames,
                        n_traj_frames=args.n_traj_frames,
                    )
                    print(f"  → eval_logit egtea (merged, partial): {acc:.2f}% (n={n_eval})")
                    if acc > best_acc:
                        best_acc = acc
                        torch.save({
        ...
        with open(log_path, "a") as f:
            f.write(json.dumps({
                ...
                "eval_acc_logit": acc, "n_eval": n_eval, "per_task": per_task,
            }) + "\n")
