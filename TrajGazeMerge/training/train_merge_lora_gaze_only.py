"""
TrajGazeMerge Stage 3: Qwen LoRA + TrajGazeV2GazeOnly (gaze-only ablation).

Identical to train_merge_lora.py except uses TrajGazeV2GazeOnly encoder.
...
Usage (single GPU — run on GPU 3):
    CUDA_VISIBLE_DEVICES=3 torchrun --nproc_per_node=1 \
        -m TrajGazeMerge.training.train_merge_lora_gaze_only \
        --stage1-ckpt /workspace/EgoGazeVQA/TrajGaze_v2/checkpoints/stage1_gaze_only/best.pth \
        --teacher-ckpt /workspace/EgoGazeVQA/TrajGazeMerge/checkpoints/baseline_lora/best.pth \
        --output-dir /workspace/EgoGazeVQA/TrajGazeMerge/checkpoints/merge_lora_gaze_only \
...
STAGE1_CKPT  = "/workspace/EgoGazeVQA/TrajGaze_v2/checkpoints/stage1_gaze_only/best.pth"
OUTPUT_ROOT  = "/workspace/EgoGazeVQA/TrajGazeMerge/checkpoints/merge_lora_gaze_only"
...
def load_traj_encoder(ckpt_path: str, device: torch.device) -> TrajGazeV2GazeOnly:
    model = TrajGazeV2GazeOnly().to(device)
    if os.path.exists(ckpt_path):
        ckpt  = torch.load(ckpt_path, map_location=device, weights_only=False)
        state = ckpt.get("model", ckpt.get("model_state_dict", ckpt))
        missing, unexpected = model.load_state_dict(state, strict=False)
        print(f"[TrajEncGazeOnly] Loaded | missing={len(missing)} unexpected={len(unexpected)}")
    else:
        print(f"[TrajEncGazeOnly] WARNING: ckpt not found: {ckpt_path}, using random init")
    return model
...
def main():
    ...
    if is_main:
        ...
        print(f"[TrajGazeMerge GazeOnly] output: {args.output_dir}")
        print(f"[TrajGazeMerge GazeOnly] GPUs={world_size}, epochs={args.epochs}, "
    ...
    if is_main:
        print("Loading TrajGazeV2GazeOnly encoder ...")
