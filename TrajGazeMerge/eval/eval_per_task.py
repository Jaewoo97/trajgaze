"""
Per-task evaluation of a trained TrajGazeMerge checkpoint on the egtea test split.

Loads best.pth (lora_state + encoder_state), runs BOTH
  - student (merged tokens, LoRA active)
  - teacher (full tokens, fine-tuned baseline LoRA)
and breaks accuracy down by task.

Single-GPU eval. Usage:
    CUDA_VISIBLE_DEVICES=0 PYTHONPATH=/workspace/trajgaze_msk \
        python -m TrajGazeMerge.eval.eval_per_task \
        --ckpt         /workspace/trajgaze_msk/TrajGazeMerge/checkpoints/improve_all/best.pth \
        ...
        --merge-ratio 0.9 --drop-ratio 0.1 --score-transform sigmoid \
        --match-score-penalty 2.0
"""
...
# Same torch.compiler shim as the training script (for older torch with newer transformers).
...
def parse_args():
    ...
    p.add_argument("--stage1-ckpt",  required=True, help="Path to TrajGaze Stage 1 ckpt")
    ...
    p.add_argument("--match-score-penalty", type=float, default=2.0)
...
def main():
    ...
    from TrajGazeMerge.models.model import (
        load_qwen_lora, load_qwen_frozen, get_option_ids, preprocess_item,
    ...
    # TrajGaze encoder (load stage1 then overlay stage2 encoder_state)
    print(f"[TrajEnc] Loading {args.stage1_ckpt}, overlaying {args.ckpt}")
    traj = TrajGazeV2().to(device)
    t_stage1 = torch.load(args.stage1_ckpt, map_location=device, weights_only=False)
    traj.load_state_dict(t_stage1.get("model", t_stage1), strict=False)
    ...
    for i, item in enumerate(ds):
        ...
        try:
            ...
            q_emb   = traj.query_encoder([item["question"]], device)
            v_feat  = traj.visual_encoder([item["traj_frame_paths"]], device)
            scores, _ = traj.encoder(traj_batch, q_emb, v_feat)
            ...
            merged_inputs = build_merged_inputs(
                base_qwen, cached, merged_video, receiver_idx, drop_idx=drop_idx
            )
