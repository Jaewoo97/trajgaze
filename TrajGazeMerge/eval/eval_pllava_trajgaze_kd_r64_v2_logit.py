"""
Logit-based eval for TrajGazeMerge (10% tokens) + PLLaVA LoRA r=64 (KD v2).
Matches training protocol exactly: checks argmax of option logits at ASSISTANT: position.

Usage:
    CUDA_VISIBLE_DEVICES=0 python -m TrajGazeMerge.eval.eval_pllava_trajgaze_kd_r64_v2_logit \
        --ckpt /workspace/EgoGazeVQA/TrajGazeMerge/checkpoints/pllava_trajgaze_kd_r64_v2/epoch_01.pth \
        --output /workspace/EgoGazeVQA/TrajGazeMerge/eval_results/pllava_trajgaze_kd_r64_v2_epoch01_logit_metrics.json
"""
...
def load_model_and_encoder(ckpt_path, device):
    ...
    if encoder_state is not None:
        ...
        stage1 = torch.load(STAGE1_CKPT, map_location="cpu", weights_only=False)
        state = stage1.get("model", stage1.get("model_state_dict", stage1))
        traj_encoder.load_state_dict(state, strict=False)
        print("[TrajEnc] loaded from stage1 fallback")
...
def get_pllava_image_features(model, pixel_values, media_type="video"):
    ...
    selected = image_outputs.hidden_states[vision_feature_layer][:, 1:]
...
def evaluate_logit(model, processor, traj_encoder, device, n_frames=16):
    ...
    with torch.no_grad():
        for i, item in enumerate(test_ds):
            ...
            try:
                ...
                proc_out = processor(text=f"{SYSTEM} USER: {IMAGE_TOKEN}\n USER: placeholder",
                ...
                merged, _ = gaze_weighted_merge(full_features[0].detach(), scores_all.detach(), r)
                merged = merged.unsqueeze(0)

                logit    = forward_logit(model, processor, merged, item["question"], item["options"], device)
...
def main():
    ...
    p.add_argument("--ckpt", default="/workspace/EgoGazeVQA/TrajGazeMerge/checkpoints/pllava_trajgaze_kd_r64_v2/epoch_01.pth")
    p.add_argument("--output", default="/workspace/EgoGazeVQA/TrajGazeMerge/eval_results/pllava_trajgaze_kd_r64_v2_epoch01_logit_metrics.json")
    ...
    print("Running logit-based eval on egtea test split (526 items) ...")
    overall, n_total, per_task_acc, per_task_n = evaluate_logit(
        model, processor, traj_encoder, device, n_frames=args.n_frames
    )

    print(f"\n=== TrajGaze KD r=64 v2 (epoch_01) — logit eval ===")
    ...
    result = {
        "model": "pllava_trajgaze_kd_r64_v2_epoch01_logit",
        "ckpt": args.ckpt,
        "eval_type": "logit_based",
