"""
Logit-based eval for TrajGazeMerge (10% tokens) + PLLaVA LoRA r=16 (KD fine-tuned).
Prompt ends at ASSISTANT:Best option:( and reads logit at that position.

Usage:
    CUDA_VISIBLE_DEVICES=2 python -m TrajGazeMerge.eval.eval_pllava_trajgaze_kd_r16_logit \
        --ckpt /workspace/EgoGazeVQA/TrajGazeMerge/checkpoints/pllava_trajgaze_kd_r16/epoch_03.pth \
        --output /workspace/EgoGazeVQA/TrajGazeMerge/eval_results/pllava_trajgaze_kd_r16_epoch03_logit_metrics.json
"""
...
def load_model_and_encoder(ckpt_path, device):
    ...
    if encoder_state is not None:
        ...
        print("[TrajEnc] WARNING: no encoder_state, using random init")
    ...
    with torch.no_grad():
        for i, item in enumerate(test_ds):
            ...
            try:
                ...
                merged = merged.to(model_dtype)

                embeds_m, mask_m, _, _, _ = model._merge_input_ids_with_image_features(
                    merged, inputs_embeds, input_ids, attn_mask, labels=None
                )
...
def main():
    ...
    p.add_argument("--output", default="/workspace/EgoGazeVQA/TrajGazeMerge/eval_results/pllava_trajgaze_kd_r16_epoch03_logit_metrics.json")
    ...
    print(f"\n=== TrajGaze KD r=16 (epoch_03) — logit eval ===")
    ...
    result = {
        "model": "pllava_trajgaze_kd_r16_epoch03_logit",
        "ckpt": args.ckpt,
