#!/usr/bin/env bash
# Sprint 2.1 (C: shuffle aug, lambda=0.2) — Diagnostic re-run on the new ckpt.
...
LORA=$REPO/TrajGazeMerge/checkpoints/E1_sprint2_1_C_keep10/best.pth
TAG=E1_sprint2_1_C
LAUNCHER=$REPO/TrajGazeMerge/eval_results/E1_sprint2_1_C_diagnostics_launcher.log
mkdir -p "$(dirname "$LAUNCHER")"

echo "[$(date -u)] Sprint2.1 diagnostics launcher waiting on Stage-2 ckpt" > "$LAUNCHER"

while true; do
    if pgrep -f "train_merge_lora_temporal_no_kd.*E1_sprint2_1_C_keep10" >/dev/null 2>&1; then
        sleep 120
        continue
    fi
    if [ -f "$LORA" ]; then
...
print(f"acc            = {acc:.2f}%   (target >= 65; Sprint1 65.59, Sprint2 61.03)")
print(f"gt_gaze_recall = {recall:.3f}   (target > 0.20; Sprint1 0.111, Sprint2 0.106)")
print(f"late_half      = {late:.3f}    (target ~ 0.50; Sprint1 0.70, Sprint2 0.70)")
print(f"shuffle_kept   Delta = {sh-b:+.2f}pp   (target < -1.5; Sprint1 +0.19, Sprint2 -2.28)")
print(f"mask_kept_early Delta = {me-b:+.2f}pp   (target < 0; Sprint2 +2.47)")
EOF
