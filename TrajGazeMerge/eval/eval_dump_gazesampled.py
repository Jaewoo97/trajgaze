"""Per-item eval dump for gaze-event temporal sampling models.

Usage:
    python -m TrajGazeMerge.eval.eval_dump_gazesampled \\
        --ckpt .../gazesampled_gaze/best.pth \\
        --arm gaze_event \\
        --dump .../dumps/gazesampled_gaze.jsonl --gpu 0
"""
...
def parse_args():
    ...
    p.add_argument("--arm",      choices=["gaze_event", "random_event", "uniform"],
                   default="gaze_event")
...
def main():
    ...
    print(f"[eval_dump_gazesampled] arm={args.arm}  ckpt={args.ckpt}", flush=True)
    ...
    with open(args.dump, "w") as fout, torch.no_grad():
        for idx in range(len(ds)):
            try:
                ...
                # Gaze-event frame selection (the Direction C contribution)
                full = item.get("full_frame_paths")
                if full:
                    frame_paths = select_frame_paths(
                        args.arm, full, item["traj"],
                        n_frames=args.n_frames,
                        n_event_frac=args.n_event_frac,
                        item_seed=_item_seed(item),
                    )
                else:
                    frame_paths = item["vlm_frame_paths"]

                n_event_actual = int(args.n_event_frac * args.n_frames)
                ...
                fout.write(json.dumps({
                    ...
                    "arm": args.arm,
                    "n_event_actual": n_event_actual,
                    "pool_size": len(full) if full else args.n_frames,
                }) + "\n")
            except Exception as e:
                print(f"  idx={idx} ERR {e!r}", flush=True)

    overall = 100.0 * correct / max(1, total)
    print(f"\n[eval_dump_gazesampled] Overall: {overall:.2f}%  (n={total})  → {args.dump}",
          flush=True)
