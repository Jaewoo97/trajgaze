...
def preprocess_batch(
    ...
    with torch.no_grad():
        # transformers >= 4.51 removed `model.get_video_features`; call the visual
        # tower directly and split the concatenated output per-sample by grid_thw.
        all_video_embeds = base_qwen.visual(pv_vid, grid_thw=grid_thw).to(emb_dev)
        # Per-sample token count = T * (H*W) / spatial_merge_unit (typically 4)
        spatial_merge_unit = int(getattr(base_qwen.visual, "spatial_merge_unit", 4))
        per_sample_n = [
            int(grid_thw[i, 0].item() * grid_thw[i, 1].item() * grid_thw[i, 2].item()
                // spatial_merge_unit)
            for i in range(grid_thw.shape[0])
        ]
        # Sanity: total tokens should equal sum of per-sample counts
        assert sum(per_sample_n) == all_video_embeds.shape[0], (
            f"per-sample count sum {sum(per_sample_n)} != "
            f"total video tokens {all_video_embeds.shape[0]}"
        )
        video_embeds = all_video_embeds
        sample_offsets = [0]
