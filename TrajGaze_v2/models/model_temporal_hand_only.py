"""
TrajGazeV2TemporalHandOnly — hand-only ablation of TrajGazeV2Temporal.

Identical to TrajGazeV2Temporal (Option C, TrajScoreHead) except:
  - SpatiotemporalEncoderTemporalHandOnly: 3 tokens/frame (left, right, bimanual)
  - TrajScoreHead pools over 3 tokens instead of 4
  - TrajectoryDecoder cross-attn memory: (B, T_past*3, D)
  - Gaze inputs in batch are silently ignored by the encoder
...
from .encoder_temporal_hand_only     import SpatiotemporalEncoderTemporalHandOnly
from .decoders                       import traj_loss
...
class TrajGazeV2TemporalHandOnly(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = SpatiotemporalEncoderTemporalHandOnly(
    ...
    def stage1_forward(self, batch: dict) -> dict[str, torch.Tensor]:
        ...
        if enc_attn is not None:
            n_tok = context.shape[2]  # 3 for hand-only
    ...
    @classmethod
    def load(cls, path: str, **kwargs) -> "TrajGazeV2TemporalHandOnly":
        model = cls(**kwargs)
