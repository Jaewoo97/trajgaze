"""
Evaluate NVILA-8B-HD-Video WITHOUT AutoGaze pruning on StreamGaze
present_future_action_prediction — all samples.

gazing_ratio_tile=1.0, gazing_ratio_thumbnail=1.0 (all patches kept).
Logs per-sample VLM inference time.

Usage:
  /workspace/vila_eval_env/bin/python eval/eval_nvila_no_gaze.py \
...
def load_model():
    print("Loading NVILA-HD-Video processor (no gaze: ratio=1.0 on tiles+thumbnails) ...")
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        for sample in tqdm(samples, desc="Evaluating (no gaze)"):
    ...
    save_results(results, out_dir, "nvila_no_gaze_32",
                 extra={
                     "gazing_ratio_tile": 1.0,
                     "gazing_ratio_thumbnail": 1.0,
