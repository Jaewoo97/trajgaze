"""
Evaluate NVILA-8B-HD-Video WITH AutoGaze on StreamGaze
...
Two protocols:
  --autogaze ntp   : NTP checkpoint  (pre-RL, trained on StreamGaze fold_b)
  --autogaze grpo  : GRPO checkpoint (RL fine-tuned on StreamGaze fold_b)
...
# ── AutoGaze checkpoints ───────────────────────────────────────────────────────
AUTOGAZE_NTP  = "/workspace/EgoGazeVQA/AutoGaze/exps/streamgaze_fold_b_ntp/checkpoint_latest_gaze"
AUTOGAZE_GRPO = "/workspace/EgoGazeVQA/AutoGaze/exps/streamgaze_fold_b_grpo/checkpoint_ep0_iter6000/checkpoint_gaze"
...
def main():
    ...
    parser.add_argument("--autogaze", choices=["ntp", "grpo"], required=True)
    ...
    autogaze_ckpt = AUTOGAZE_NTP if args.autogaze == "ntp" else AUTOGAZE_GRPO
    out_dir = os.path.abspath(args.output)
    stem    = f"nvila_with_autogaze_{args.autogaze}"
    ...
    processor, model = load_model(autogaze_ckpt)

    results, skipped = [], 0
    with tempfile.TemporaryDirectory() as tmp_dir:
        for sample in tqdm(samples, desc=f"Evaluating (autogaze={args.autogaze})"):
    ...
    save_results(results, out_dir, stem, extra={"autogaze": args.autogaze, "skipped": skipped})
