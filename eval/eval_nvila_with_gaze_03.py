"""
Evaluate NVILA-8B-HD-Video WITH AutoGaze pruning (ratio=0.3) on StreamGaze
present_future_action_prediction — all samples.

gazing_ratio_tile=0.3, gazing_ratio_thumbnail=0.3 (prune both streams).
Logs per-sample VLM inference time.

Usage:
  /workspace/vila_eval_env/bin/python eval/eval_nvila_with_gaze_03.py \
...
def load_model():
    ...
    print("Loading NVILA-HD-Video processor (gaze ratio=0.3 on tiles+thumbnails) ...")
    processor = AutoProcessor.from_pretrained(
        MODEL_PATH,
        num_video_frames=128,
        num_video_frames_thumbnail=128,
        max_tiles_video=1,
        gazing_ratio_tile=0.3,
        gazing_ratio_thumbnail=0.3,
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        for sample in tqdm(samples, desc="Evaluating (gaze=0.3)"):
    ...
    save_results(results, out_dir, "nvila_with_gaze_03",
                 extra={
