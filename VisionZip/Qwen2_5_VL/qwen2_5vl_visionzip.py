    ...
    def forward(
        ...
        if return_logits:
            with torch.no_grad():
                ...
                H, T, D = qh.shape
                scale = D ** 0.5
                # Chunked over query rows to avoid materializing the [H, T, T]
                # softmax map (OOMs on long video seqs). softmax is over keys,
                # so query-row chunking is exact. We return per-head key-wise
                # column sums shaped [H, 1, T]; the encoder's
