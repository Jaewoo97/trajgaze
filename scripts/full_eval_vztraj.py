"""Full per-source eval for VZ-traj (VisionZip-traj) best.pth — same protocol as
the VisionZip / TAS full evals, but using the trajectory-anchored VisionZip
selection rule `select_with_traj` (imported verbatim from the VZ-traj trainer so
there is zero drift from training). Output JSON next to the ckpt.

Usage:
    CUDA_VISIBLE_DEVICES=0 /opt/conda/envs/trajgaze/bin/python -m scripts.full_eval_vztraj \\
        --ckpt /workspace/.../visionzip_traj_lora_3way_overlay/best.pth --gpu 0
"""
...
# Must be first so the TrajGazeMerge package path locks to the current branch
# before train_visionzip_lora inserts the stale /workspace/EgoGazeVQA path.
...
# Exact selection rule used at train time (VisionZip attn weighted by gaze/hand traj)
from TrajGazeMerge.training.train_visionzip_traj_lora_3way import select_with_traj
...
def parse_args():
    ...
    # Trajectory weighting hyperparameters — defaults match the overlay launch
    # (launch_vz_traj_overlay.sh did not override any of these).
...
def main():
    ...
    print(f"[full_eval_vztraj] loading VisionZip Qwen2.5-VL-7B + LoRA on cuda:{args.gpu}",
          flush=True)
    print(f"[full_eval_vztraj] hp: σ_g={hp['sigma_g']} σ_h={hp['sigma_h']} "
          f"α_hand={hp['alpha_hand']} σ_v={hp['sigma_v']} σ_gh={hp['sigma_gh']}", flush=True)
    processor, model = load_visionzip_lora(device)
    base_qwen = model.get_base_model()
    option_ids = get_option_ids(processor, 5)

    print(f"[full_eval_vztraj] loading LoRA ckpt: {args.ckpt}", flush=True)
    ckpt = torch.load(args.ckpt, map_location=device, weights_only=False)
    if "lora_state" in ckpt:
        model.load_state_dict(ckpt["lora_state"], strict=False)
    model.eval()

    print("[full_eval_vztraj] loading full 3-way test set", flush=True)
    test_ds = CombinedMergeDataset(
        split="test", n_vlm_frames=128, n_traj_frames=128, include_hdepic=True,
    )
    print(f"[full_eval_vztraj] {len(test_ds)} items", flush=True)
    ...
    for idx in range(args.start_idx, end_idx):
        ...
        try:
            ...
            selected_embeds, receiver_idx = select_with_traj(cached, item, device, hp)
    ...
    print("\n=== FINAL FULL EVAL (VZ-traj) ===", flush=True)
