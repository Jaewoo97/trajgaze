...
class PllavaForConditionalGeneration(PllavaPreTrainedModel):
    ...
    def _merge_input_ids_with_image_features(self, image_features, inputs_embeds, input_ids, attention_mask, labels):
        ...
        # Original code located image slots via "all-zero rows", which breaks
        # when pad_token_id == 0 (Llama-2 unk) because embed_tokens(0) is itself
        # near-zero so right-padding rows look like image-fill rows. Compute
        # image positions explicitly as the complement of text positions.
        image_to_overwrite = torch.ones(
            (batch_size, max_embed_dim), dtype=torch.bool, device=target_device
        )
    ...
    def merge_frames_dynamic(self, frames, threshold=0.8, k=7):
        B, L, C = frames.shape
        # Projector output has temporal dim == pooling_shape[0] (post-pool),
        # NOT num_frames. Original assertion only happens to hold when
        # num_frames == pooling_shape[0] (e.g. 16-frame default).
        pool_T = self.config.pooling_shape[0]
        assert L == pool_T * self.config.pooling_shape[1] * self.config.pooling_shape[2], \
            f"L={L}, expected {pool_T * self.config.pooling_shape[1] * self.config.pooling_shape[2]} " \
            f"(pool_T={pool_T}, pool_H={self.config.pooling_shape[1]}, pool_W={self.config.pooling_shape[2]})"
        frames = frames.view(B, pool_T, self.config.pooling_shape[1]*self.config.pooling_shape[2], C) # B T L C
        idx_clusters, _ = cluster_dpc_knn(frames.mean(dim=2), cluster_num=int(pool_T*self.config.temporal_segment_ratio), k=k)
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None and input_ids.shape[1] != 1:
                ...
                if labels is None:
                    # In eval/generate we keep labels None; otherwise a
                    # full_like(attention_mask) label would collide with logits
                    # that have already been shortened by visual-token pruning /
                    # pooling, triggering IndexError on shift_attention_mask.
    ...
    def generate(
        ...
        **kwargs,
        ...
        # Stash media_type so prepare_inputs_for_generation can recover it even
        # if transformers strips it from model_kwargs after validate.
        self._pending_media_type = kwargs.get('media_type', 'video')
    ...
    def prepare_inputs_for_generation(
        ...
        # transformers >=4.42 strips unexpected kwargs before prepare_inputs_for_generation,
        # so fall back to the last media_type the caller set via self._pending_media_type.
        media_type = kwargs.get('media_type', None) or getattr(self, '_pending_media_type', 'video')

        # After the first step, pixel_values have already been projected into inputs_embeds;
        # re-feeding them re-runs the vision tower with media_type lost — skip instead.
        if past_key_values is not None:
            pixel_values = None
