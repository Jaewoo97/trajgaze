...
def setup_model(
    ...
    extra_pllava_kwargs = {}
    for k in ('selected_layer', 'alpha', 'head', 'softmax',
              'tau', 'cluster_ratio', 'temporal_segment_ratio'):
        if hasattr(config.model, k):
            extra_pllava_kwargs[k] = getattr(config.model, k)
    ...
    if not config.model.freeze_lm:
        for n, p in model.language_model.named_parameters():
            for key in config.model.lm_unfreeze_modules:
                if key in n:
    ...
    if config.model.use_lora:
        ...
        # PEFT + gradient_checkpointing requires the LM input embedding to flow
        # grad so that torch.utils.checkpoint sees a requires_grad input. Without
        # this, LoRA parameters get no gradient and loss stays flat.
        if config.gradient_checkpointing and hasattr(model.language_model, 'enable_input_require_grads'):
    ...
    if config.model.pretrained_path is not None and not config.deepspeed:
        ...
        # Drop checkpoint-side LoRA weights whose shape mismatches the live model
        # (e.g. PLLaVA-7B safetensors stores r=128 LoRA, our config trains r=16).
        # Keeping them would raise on load even with strict=False. Base/projector
        # weights remain loaded; LoRA reinitializes from peft_config.
        ...
        for k, v in state_dict.items():
            ...
            filtered_state[k] = v
        if skipped:
            logger.info(f"Skipped {skipped} ckpt keys with shape mismatch (likely LoRA r mismatch).")
